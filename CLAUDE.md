...
| `bci` | SCRAPER · PASSWORD | **pending** — rework + **fix post-test #1** (2026-06-13, `bci_scraper.py`): portal `www.bci.cl` (widget) + BFF v3.2. Test #1: **login OK** (post-fill verificado) + **bodies confirmados**. JWT se captura por host `apilocal.bci.cl` (sin depender del menú; el dashboard lo dispara solo); bodies reales: `por-rut {"rut":"<rut>-<dv>"}`, saldo `{"cuentaNumero":"<n>"}`, movs `{"numeroCuenta":"<n>"}` (keys distintas, sin `tipo`). RUT `type()` en `#rut_aux`, clave `fill()` en `#clave`. 2FA Digital Pass. Pendiente: test #2 headful + sync prod → activar. |
...
| **B-2** | Scraper BCI — rework + fix post-test #1, pendiente prod | Portal `www.bci.cl` (widget) + BFF v3.2; `bci_scraper.py` reconstruido y gated. **R-2 cerrado** (`BCIScraperSource`, `scraper.bci` intacto). **Test #1 (2026-06-13): login OK + bodies confirmados** → fix dirigido aplicado: JWT por host `apilocal.bci.cl` (sin depender del menú), bodies reales `por-rut {"rut":"<rut>-<dv>"}` · saldo `{"cuentaNumero"}` · movs `{"numeroCuenta"}` (sin `tipo`), capture-replay como fallback. Falta: **test #2 headful** (listar cuentas + movs) + sync prod → activar `bci`. Riesgo: DetectCA easysol (tipo B-1). |
