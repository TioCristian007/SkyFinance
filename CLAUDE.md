...
- **Fase 2 (renombre + nombre canónico)** CONSTRUIDA: `merchant_aliases` (per-user) + `merchant_display_names` (consenso global) en **migración 015 (⚠️ pendiente de aplicar)**. Endpoint `PATCH …/{id}/merchant`, display resuelto al leer (`merchant_display_batch`: propio → global → Title Case), UI con lápiz en Movimientos. `merchant_display_names` sin guarda de prioridad (no hay escritor IA, solo el consenso escribe). Guarda de lectura: keys de transferencia jamás consultan el global.
- **⚠️ Orden de deploy de la 015** (como 013/014): preflight `to_regclass` de ambas tablas → NULL · 015 en staging → `audit_rls_policies.py` (exit 0) → 015 en prod + re-audit → deploy Railway api+worker. El código viejo no lee las tablas nuevas; el nuevo las necesita.
