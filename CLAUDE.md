...
| `bci` | SCRAPER · PASSWORD | **pending** — rework + test #1 (bodies) + **test #2 + instrumentación** (2026-06-13, `bci_scraper.py`): portal `www.bci.cl` (widget) + BFF v3.2. **Login OK** end-to-end (post-fill verificado). Bodies reales: `por-rut {"rut":"<rut>-<dv>"}`, saldo `{"cuentaNumero":"<n>"}`, movs `{"numeroCuenta":"<n>"}` (keys distintas, sin `tipo`). RUT `type()` en `#rut_aux`, clave `fill()` en `#clave`. 2FA Digital Pass. **Test #2: el JWT NO se captura** — la home no pega `apilocal` hasta navegar; agregada instrumentación de diagnóstico (censo de red + sonda storage/cookies + captura DOM en el timeout, todo PII-safe/§20 y gated tras `scraper_debug_capture`) + nudge multi-frame + ventana 30s con re-nudge. Pendiente: **test #3 headful con `SCRAPER_DEBUG_CAPTURE=true`** → leer diagnóstico → fix real → sync prod → activar. |
...
| **B-2** | Scraper BCI — login OK, falta capturar el JWT (instrumentado) | Portal `www.bci.cl` (widget) + BFF v3.2; `bci_scraper.py` reconstruido y gated. **R-2 cerrado** (`BCIScraperSource`, `scraper.bci` intacto). **Login OK end-to-end** + bodies confirmados (test #1). **Test #2 (2026-06-13): el JWT no se captura** — la home no pega `apilocal` hasta navegar al view de cuentas (el supuesto "lo dispara solo" era falso). Agregada **instrumentación de diagnóstico** (censo de red + sonda storage/cookies + captura DOM en el timeout, PII-safe §20, gated tras `scraper_debug_capture`) + nudge multi-frame + ventana 30s con re-nudge a mitad. Falta: **test #3 headful con `SCRAPER_DEBUG_CAPTURE=true`** → leer `bci_network_census` / `bci_token_probe_*` / captura `jwt_timeout` → fix real → sync prod → activar `bci`. Riesgo: DetectCA easysol (tipo B-1). |
