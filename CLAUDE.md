...
| `bci` | SCRAPER · PASSWORD | **pending** — rework + tests #1/#2/#3 (2026-06-13, `bci_scraper.py`): portal `www.bci.cl` (widget) + app JSF + BFF v3.2. **Login OK** end-to-end. Bodies reales: `por-rut {"rut":"<rut>-<dv>"}`, saldo `{"cuentaNumero":"<n>"}`, movs `{"numeroCuenta":"<n>"}` (keys distintas, sin `tipo`). RUT `type()` en `#rut_aux`, clave `fill()` en `#clave`. 2FA Digital Pass. **Test #3 — ROOT CAUSE encontrado y arreglado**: el censo de red mostró que `apilocal.bci.cl` se llama con `auth_scheme='bearer'` (MINÚSCULA), pero `_is_jwt_request` matcheaba `"Bearer "` case-sensitive → el token estaba en cada request y se dejaba pasar. Fix: match case-insensitive. El JWT lo dispara la **navegación** a Saldos/Movimientos, no la home; el token NO vive en storage (va en el header). Pendiente: **test #4** (debería listar cuentas+movs+saldo) → sync real prod (vigilar DetectCA easysol, tipo B-1) → activar `bci`. |
